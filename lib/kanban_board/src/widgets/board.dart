...
class KanbanBoardScrollController {
  KanbanBoardState? _boardState;
...
class KanbanBoard extends StatelessWidget {
  const KanbanBoard({
    ...
    this.config = const KanbanBoardConfig(),
  ...
  /// the [KanbanBoardController] for more information.
  ///
  ...
  ///
  final KanbanBoardConfig config;

  /// A controller is used to control each group scroll actions.
  ///
  final KanbanBoardScrollController? boardScrollController;
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider.value(
      value: controller,
      child: Consumer<KanbanBoardController>(
        builder: (context, notifier, child) {
          final boardState = KanbanBoardState();
...
class _KanbanBoardContent extends StatefulWidget {
  ...
  final KanbanBoardScrollController? scrollManager;
  final KanbanBoardState boardState;
  final BoxConstraints groupConstraints;
  final KanbanBoardCardBuilder cardBuilder;
...
class _KanbanBoardContentState extends State<_KanbanBoardContent> {
  final GlobalKey _boardContentKey =
      GlobalKey(debugLabel: '$_KanbanBoardContent overlay key');
  ...
  Widget? _buildHeader(BuildContext context, KanbanGroupData groupData) {
    if (widget.headerBuilder == null) {
      return null;
    }
    return Selector<KanbanGroupController, KanbanGroupHeaderData>(
...
class _BoardGroupDataSourceImpl extends KanbanGroupDataDataSource {
...
class KanbanBoardState extends DraggingStateStorage
  ...
  /// Quick access to the [KanbanBoardGroup], the [GlobalKey] is bind to the
  /// KanbanBoardGroup's [ReorderFlex] widget.
